"""Tests for the FriendlySceneFlipSelect entity."""
...
async def test_set_scene_reactivates_active_slot(
    ...
    hass.states.async_set(
        "scene.evening", "scening", {"friendly_name": "Evening"}
    )
    ...
    with patch.object(
        ...
        mock_call.assert_called_once_with(
            "scene", "turn_on", {"entity_id": "scene.evening"}, blocking=True
        )

    assert entity._scene_a == "scene.evening"


async def test_set_scene_inactive_slot_no_activation(
    hass: HomeAssistant, setup_entry
) -> None:
